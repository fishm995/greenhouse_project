...
# Dictionary mapping each unique session (uniqueID) to the active connection count.
# For example: { 'uniqueID123': 2 } means that the session with uniqueID123 has 2 open connections.
session_counters = {}

# Dictionary mapping Socket.IO connection IDs to their corresponding unique session IDs.
# For example: { 'socket_id1': 'uniqueID123', 'socket_id2': 'uniqueID123' }
session_map = {}
        ...
        for sensor in sensor_configs:
            try:
                ...
                if isinstance(value, dict):
                  sensor_readings[f"{sensor.sensor_name}_Temp"]  = value["temperature"]
                  sensor_readings[f"{sensor.sensor_name}_Humid"] = value["humidity"]
                else:
...
def handle_connect():
    ...
    if unique_viewers == 1 and not ffmpeg_controller.is_ffmpeg_ready():
        ...
        # Otherwise, if FFmpeg is already running, notify this client that the stream is ready.
...
def schedule_stop_ffmpeg():
    ...
    with stop_timer_lock:
        if ffmpeg_stop_timer is None:
            # Wait 60 seconds before stopping FFmpeg.
...
def delayed_stop():
    global ffmpeg_stop_timer
    with sessions_lock:
        # Compute the unique viewer count.
        unique_viewers = len(session_counters)
    if unique_viewers == 0:
